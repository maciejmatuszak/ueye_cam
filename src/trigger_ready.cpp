...
public:
    TriggerReady()
    {
        ...
        n_.getParam("cameras", camer_names_);

        ROS_INFO_STREAM("parsed cameras: " << camer_names_.size());
        for(vector<string>::iterator it = camer_names_.begin(); it != camer_names_.end(); ++it) {
            camera_status_[*it]=false;
            ROS_INFO_STREAM("adding camera : " << *it);
        }
        advertiseService();
    }
